...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Get current item
    const currentItem = await db.bPD.findFirst({
    ...
    if (direction === 'up' && currentIndex > 0) {
      ...
      await db.$transaction([
        db.bPD.update({ where: { id }, data: { urutan: prevItem.urutan } }),
        db.bPD.update({ where: { id: prevItem.id }, data: { urutan: currentItem.urutan } })
      ]);
    } else if (direction === 'down' && currentIndex < allItems.length - 1) {
      const nextItem = allItems[currentIndex + 1];
      await db.$transaction([
        db.bPD.update({ where: { id }, data: { urutan: nextItem.urutan } }),
        db.bPD.update({ where: { id: nextItem.id }, data: { urutan: currentItem.urutan } })
      ]);
    } else {
      return NextResponse.json({
        success: false,
