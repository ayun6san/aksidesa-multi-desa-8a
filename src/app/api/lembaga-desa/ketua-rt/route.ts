...
// GET - Fetch all Ketua RT members
export async function GET(request: NextRequest) {
  try {
    ...
    const ketuaRT = await db.ketuaRT.findMany({
      where,
      orderBy: [
        { urutan: 'asc' },
        { jabatan: 'asc' },
...
// POST - Create new Ketua RT member
export async function POST(request: NextRequest) {
  try {
    ...
    const maxUrutan = await db.ketuaRT.aggregate({
      where: { desaId },
      _max: { urutan: true }
    });
    const nextUrutan = (maxUrutan._max.urutan || 0) + 1;

    const ketuaRT = await db.ketuaRT.create({
      data: {
        desaId,
    ...
    console.error('Error creating Ketua RT:', error);
    return NextResponse.json(
      { success: false, error: 'Gagal menambahkan data Ketua RT' },
      { status: 500 }
    );
  }
}
