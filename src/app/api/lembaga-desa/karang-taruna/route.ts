...
// GET - Fetch all Karang Taruna members
export async function GET(request: NextRequest) {
  try {
    ...
    const karangTaruna = await db.karangTaruna.findMany({
      where,
      orderBy: [
        { urutan: 'asc' },
        { jabatan: 'asc' },
...
// POST - Create new Karang Taruna member
export async function POST(request: NextRequest) {
  try {
    ...
    const maxUrutan = await db.karangTaruna.aggregate({
      where: { desaId },
      _max: { urutan: true }
    });
    const nextUrutan = (maxUrutan._max.urutan || 0) + 1;
    
    const karangTaruna = await db.karangTaruna.create({
      data: {
        desaId,
    ...
    console.error('Error creating Karang Taruna:', error);
    return NextResponse.json(
      { success: false, error: 'Gagal menambahkan data Karang Taruna' },
      { status: 500 }
    );
  }
}
