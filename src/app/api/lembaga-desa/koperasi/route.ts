...
// GET - Fetch all Koperasi members
export async function GET(request: NextRequest) {
  try {
    ...
    const koperasi = await db.koperasi.findMany({
      where,
      orderBy: [{ urutan: 'asc' }, { jabatan: 'asc' }, { namaLengkap: 'asc' }]
    });
...
// POST - Create new Koperasi member
export async function POST(request: NextRequest) {
  try {
    ...
    if (!desaAccess.desaId) {
      return NextResponse.json({ success: false, error: 'Anda tidak terikat ke desa manapun' }, { status: 400 });
    }

    const body = await request.json();
    const desaId = user.role === 'SUPER_ADMIN' && body.desaId ? body.desaId : desaAccess.desaId;

    const maxUrutan = await db.koperasi.aggregate({ where: { desaId }, _max: { urutan: true } });
    const nextUrutan = (maxUrutan._max.urutan || 0) + 1;

    const koperasi = await db.koperasi.create({
      data: {
        desaId,
    ...
    return NextResponse.json({ success: true, data: koperasi, message: 'Data Koperasi berhasil ditambahkan' });
  } catch (error) {
    console.error('Error creating Koperasi:', error);
    return NextResponse.json({ success: false, error: 'Gagal menambahkan data Koperasi' }, { status: 500 });
  }
}
