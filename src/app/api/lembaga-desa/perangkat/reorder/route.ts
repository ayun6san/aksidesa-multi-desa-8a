...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Get current item
    const currentItem = await db.perangkatDesa.findFirst({
    ...
    if (direction === 'up' && currentIndex > 0) {
      ...
      await db.$transaction([
        db.perangkatDesa.update({ where: { id }, data: { urutan: prevItem.urutan } }),
        db.perangkatDesa.update({ where: { id: prevItem.id }, data: { urutan: currentItem.urutan } })
      ]);
    } else if (direction === 'down' && currentIndex < allItems.length - 1) {
      const nextItem = allItems[currentIndex + 1];
      await db.$transaction([
        db.perangkatDesa.update({ where: { id }, data: { urutan: nextItem.urutan } }),
        db.perangkatDesa.update({ where: { id: nextItem.id }, data: { urutan: currentItem.urutan } })
      ]);
    } else {
      return NextResponse.json({
        success: false,
