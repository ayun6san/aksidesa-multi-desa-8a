...
export async function GET(request: NextRequest) {
  try {
    ...
    const bumdes = await db.bumdes.findMany({
      where,
      orderBy: [
        { urutan: 'asc' },
        { jabatan: 'asc' },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxUrutan = await db.bumdes.aggregate({
      where: { desaId },
      _max: { urutan: true }
    });
    const nextUrutan = (maxUrutan._max.urutan || 0) + 1;

    const bumdes = await db.bumdes.create({
      data: {
        desaId,
    ...
    return NextResponse.json(
      { success: false, error: 'Gagal menambahkan data Bumdes' },
      { status: 500 }
    );
  }
}
